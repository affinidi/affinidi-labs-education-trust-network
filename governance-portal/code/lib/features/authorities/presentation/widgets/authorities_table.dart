...
class AuthoritiesTable extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var filteredAuthorities = authorities.where((authority) {
      if (searchQuery.value.isEmpty) return true;
      final query = searchQuery.value.toLowerCase();
      return authority.name.toLowerCase().contains(query) ||
          authority.did.toLowerCase().contains(query) ||
          (authority.description?.toLowerCase().contains(query) ?? false);
    }).toList();

    // Sort authorities
    filteredAuthorities.sort((a, b) {
              ...
              Container(
                ...
                child: DropdownButton<String>(
                  ...
                  items: [
                    ...
                    DropdownMenuItem(
                      value: 'did-asc',
                      child: Text('Authority ID (A-Z)',
                          style: AppTypography.bodyMedium),
                    ),
                    DropdownMenuItem(
                      value: 'did-desc',
                      child: Text('Authority ID (Z-A)',
                          style: AppTypography.bodyMedium),
                    ),
                  ],
                ),
                      ...
                      child: ConstrainedBox(
                        ...
                        child: DataTable(
                          ...
                          columns: const [
                            ...
                            DataColumn(label: Text('Authority ID')),
                          ...
                          rows: List<DataRow>.generate(
                            ...
                            // Find authority for this row
                            final authorityDid =
                                (row.cells[1].child as _EllipsizedCell).text;
                            final authority = filteredAuthorities
                                .firstWhere((a) => a.did == authorityDid);
                            final recordKey = authority.id;
                            final isExpanded =
                                expandedRows.value.contains(recordKey);
